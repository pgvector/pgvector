...
Note: with the default `fast_rotation`, dimensions are padded to the next power of two, so the practical limit is 8,192; use `fast_rotation = false` to index up to 16,000 dimensions
...
> **Note:** TQIVF is recommended for datasets up to roughly 10 million rows. Like any IVF index, it scans a fixed fraction of the vectors per query (approximately `probes × rows / lists`), so the number of vectors examined — and therefore query latency — grows with the table size at a fixed recall. For larger datasets, use [TQHNSW](#tqhnsw) instead: its graph search is sublinear in the number of rows, so it sustains low query latency at high recall while using a fraction of the memory of HNSW.
...
Note: like IVFFlat, each list stores a full-precision centroid that must fit on an index page, so these limits are lower than the other TurboQuant index types
