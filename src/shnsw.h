...
#define HNSW_ELEMENT_TUPLE_SIZE(_n_elem)	MAXALIGN(offsetof(HnswElementTupleData, vec) + VECTOR_SIZE(_n_elem))
...
typedef struct HnswBuildState
{
	...
	double		indtuples;
	double		reltuples;
	double 		usedmem; // TODO: should this be a double or something else?
...
IndexBuildResult *shnswbuild(Relation heap, Relation index, IndexInfo *indexInfo);
void		shnswbuildempty(Relation index);
bool		shnswinsert(Relation index, Datum *values, bool *isnull, ItemPointer heap_tid, Relation heap, IndexUniqueCheck checkUnique
#if PG_VERSION_NUM >= 140000
					   ,bool indexUnchanged
#endif
					   ,IndexInfo *indexInfo
);
IndexBulkDeleteResult *shnswbulkdelete(IndexVacuumInfo *info, IndexBulkDeleteResult *stats, IndexBulkDeleteCallback callback, void *callback_state);
IndexBulkDeleteResult *shnswvacuumcleanup(IndexVacuumInfo *info, IndexBulkDeleteResult *stats);
IndexScanDesc shnswbeginscan(Relation index, int nkeys, int norderbys);
void		shnswrescan(IndexScanDesc scan, ScanKey keys, int nkeys, ScanKey orderbys, int norderbys);
bool		shnswgettuple(IndexScanDesc scan, ScanDirection dir);
void		shnswendscan(IndexScanDesc scan);
