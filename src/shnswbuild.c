...
CreateElementPages(HnswBuildState * buildstate)
{
	...
	foreach(lc, buildstate->elements)
	{
		...
		// Tuple size can vary, so have to allocate for each element
		etupSize = HNSW_ELEMENT_TUPLE_SIZE(element->vec->n_elem);
...
static double
HnswGetElementMemorySize(int m, double ml, int n_elem)
{
	...
	elementSize += VECTOR_SIZE(n_elem);
	return elementSize;
}
...
InsertTuple(Relation index, Datum *values, HnswElement element, HnswBuildState * buildstate, HnswElement * dup)
{
	...
	{
		if (!HnswNormValue(buildstate->normprocinfo, collation, &value, NULL))
			return false;
	}

	Vector* vec = DatumGetVector(value);
	/* Copy value to element so accessible outside of memory context */
	memcpy(element->vec, vec, VECTOR_SIZE(vec->n_elem));
	...
	*dup = HnswFindDuplicate(element);
	...
	buildstate->usedmem += HnswGetElementMemorySize(buildstate->m, buildstate->ml, element->vec->n_elem);
			  ...
			  bool *isnull, bool tupleIsAlive, void *state)
{
	...
	if (buildstate->usedmem >= buildstate->maxInMemorySize)
	{
		if (!buildstate->flushed)
		{
			ereport(NOTICE,
					(errmsg("shnsw graph no longer fits into maintenance_work_mem after " INT64_FORMAT " tuples", (int64) buildstate->indtuples),
		...
		if (HnswInsertTuple(buildstate->index, values, isnull, tid, buildstate->heap)) {
			UpdateProgress(PROGRESS_CREATEIDX_TUPLES_DONE, ++buildstate->indtuples);
			buildstate->usedmem += HnswGetElementMemorySize(buildstate->m, buildstate->ml, DatumGetVector(value)->n_elem);
		}
	...
	element->vec = palloc(VECTOR_SIZE(DatumGetVector(value)->n_elem));
...
static double
HnswGetMaxInMemorySize() {
	return maintenance_work_mem * 1024L;
}
...
InitBuildState(HnswBuildState * buildstate, Relation heap, Relation index, IndexInfo *indexInfo, ForkNumber forkNum)
{
	...
	buildstate->maxInMemorySize = HnswGetMaxInMemorySize();
 ...
 */
IndexBuildResult *
shnswbuild(Relation heap, Relation index, IndexInfo *indexInfo)
{
 ...
 */
void
shnswbuildempty(Relation index)
{
