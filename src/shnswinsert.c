...
WriteNewElementPages(Relation index, HnswElement e, int m, BlockNumber insertPage, BlockNumber *updatedInsertPage)
{
	...
	/* Calculate sizes */
	etupSize = HNSW_ELEMENT_TUPLE_SIZE(n_elem);
...
HnswAddDuplicate(Relation index, HnswElement element, HnswElement dup)
{
	...
	Page		page;
	GenericXLogState *state;
	Size		etupSize = HNSW_ELEMENT_TUPLE_SIZE(dup->vec->n_elem);
	HnswElementTuple etup;
	int			i;
 ...
 */
bool
shnswinsert(Relation index, Datum *values, bool *isnull, ItemPointer heap_tid,
