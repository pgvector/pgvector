...
HnswNormValue(FmgrInfo *procinfo, Oid collation, Datum *value, Vector * result)
{
	...
	if (norm > 0)
	{
		...
		if (result == NULL)
			result = InitVector(v->n_elem, v->dim);

		for (int i = 0; i < v->n_elem; i++) {
			result->x[i].index = v->x[i].index;
			result->x[i].value = v->x[i].value / norm;
		}
...
HnswSetElementTuple(HnswElementTuple etup, HnswElement element)
{
	...
	memcpy(&etup->vec, element->vec, VECTOR_SIZE(element->vec->n_elem));
}
...
HnswLoadElementFromTuple(HnswElement element, HnswElementTuple etup, bool loadHeaptids, bool loadVec)
{
	...
	if (loadVec)
	{
		element->vec = palloc(VECTOR_SIZE(etup->vec.n_elem));
		memcpy(element->vec, &etup->vec, VECTOR_SIZE(etup->vec.n_elem));
	}
}
...
HnswSearchLayer(Datum q, List *ep, int ef, int lc, Relation index, FmgrInfo *procinfo, Oid collation, int m, bool inserting, HnswElement skipElement)
{
	...
	v = hash_create("shnsw visited", 256, &hash_ctl, HASH_ELEM | HASH_BLOBS | HASH_CONTEXT);
...
HnswFindDuplicate(HnswElement e)
{
	...
	for (int i = 0; i < neighbors->length; i++)
	{
		...
		if (svector_cmp_internal(e->vec, neighbor->element->vec) != 0)
			break;
