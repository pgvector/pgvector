	...
	while (BlockNumberIsValid(blkno))
	{
		...
		for (offno = FirstOffsetNumber; offno <= maxoffno; offno = OffsetNumberNext(offno))
		{
			...
			if (ItemPointerIsValid(&etup->heaptids[0]))
			{
				...
				if (itemUpdated)
				{
					Size		etupSize = HNSW_ELEMENT_TUPLE_SIZE(etup->vec.n_elem);
...
MarkDeleted(HnswVacuumState * vacuumstate)
{
	...
	while (BlockNumberIsValid(blkno))
	{
		...
		for (offno = FirstOffsetNumber; offno <= maxoffno; offno = OffsetNumberNext(offno))
		{
			...
			/* Calculate sizes */
			etupSize = HNSW_ELEMENT_TUPLE_SIZE(etup->vec.n_elem);
			...
			/* Overwrite element */
			etup->deleted = 1;
			MemSet(&etup->vec.x, 0, etup->vec.n_elem * sizeof(VecEl));
...
InitVacuumState(HnswVacuumState * vacuumstate, IndexVacuumInfo *info, IndexBulkDeleteResult *stats, IndexBulkDeleteCallback callback, void *callback_state)
{
	...
	vacuumstate->deleted = hash_create("shnswbulkdelete indextids", 256, &hash_ctl, HASH_ELEM | HASH_BLOBS | HASH_CONTEXT);
}
...
shnswbulkdelete(IndexVacuumInfo *info, IndexBulkDeleteResult *stats,
 ...
 */
IndexBulkDeleteResult *
shnswvacuumcleanup(IndexVacuumInfo *info, IndexBulkDeleteResult *stats)
{
