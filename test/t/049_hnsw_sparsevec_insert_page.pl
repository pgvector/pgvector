...
# Scattered deletes leave freed small slots on early pages, and VACUUM points
# insertPage at the first of them
...
# The first large insert can pay once to walk past the freed slots, but later
# ones must not - insertPage cannot stay on a page too small to hold them
insert_buffers($node, $large_nnz, 900002);
my $repeat = insert_buffers($node, $large_nnz, 900003);

# A small insert must not send large inserts back to scanning the index
...
# An insert that walks to the end of the index reads at least $pages buffers.
# Compare against the index size rather than an absolute count so the test does
