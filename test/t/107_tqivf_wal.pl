# Based on test/t/010_hnsw_wal.pl (in turn based on contrib/bloom/t/001_wal.pl)

# Test generic xlog record work for tqivf index replication.
use strict;
...
sub test_index_replay
{
	...
	# Probe every list so primary and replica score the same candidate set.
	my $queries = qq(
...
$node_primary->safe_psql("postgres",
	"CREATE INDEX ON tst USING tqivf (v vector_l2_ops) WITH (lists = 10);");
